...
std::string constructErrorMessage(char const* file, unsigned line, char const* function, std::string msg)
{
  assert(file);
  assert(function);
  ...
  // "<file>:<line> <msg> (in <func>)" + '\0'
  const     auto totalLen = fileLen + 1 + numLen + 1 + msg.size() + 5 + funcLen + 1 + 1;
...
Exception::Exception(char const* file, unsigned line, char const* function, std::string msg):
  std::runtime_error{ constructErrorMessage(file, line, function, std::move(msg)) }
{ }
