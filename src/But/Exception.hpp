...
#define BUT_DEFINE_EXCEPTION(name, base, msgDef) \
  ...
  { \
    name(char const* file, unsigned line, char const* func, std::string msg): \
      base(file, line, func, msgDef, std::move(msg)) \
    { } \
    name(char const* file, unsigned line, char const* func, char const* derivedMsgDef, std::string msg): \
      base(file, line, func, derivedMsgDef, std::move(msg)) \
    { } \
  }
